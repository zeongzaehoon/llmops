...
### Backend
`Python` `FastAPI` `Gunicorn` `Uvicorn` `asyncio` `Motor(MongoDB)` `Redis` `Pinecone` `aioboto3(AWS S3)` `JWT` `FastMCP` `Pydantic`

### Infra / DevOps
`Docker` `Docker Compose` `Nginx` `Slack Webhook`

### AI / ML
`OpenAI API` `Anthropic API` `Azure OpenAI` `AWS Bedrock` `Google Gemini` `Pinecone(Vector DB)` `Milvus(Vector DB)` `MCP(Model Context Protocol)`
...
│         │                                               │
│  ┌──────▼──────────────────┐  ┌──────────────────────┐  │
│  │  RunLLM (Single Step)   │  │  RunMCP (Multi Step) │  │
...
### 0. 요약
```Summarize
[프로젝트] 멀티 LLM 프로바이더 추상화 계층 설계(llm-proxy)
  + 소스코드 배포없이 UI에서 에이전트 생성하고 테스트 가능하도록 구현 
[프로젝트] 동기(flask) -> 비동기(fastapi) 기반 고성능 데이터 오케스트레이션(api)
  + llm 요청까지 소요시간 10초 -> 3초로 감소
[프로젝트] 에이전트 확장 가능한 설계(api)
  + 실 서비스에서 24개 이상의 에이전트를 관리
[프로젝트] MCP (Model Context Protocol) 에이전트 통합(api, llm-proxy)
  + ui에서 에이전트에 동적으로 MCP 서버 등록, 서버 선택, 툴 조회, 툴 등록할 수 있도록 DB 설계 및 API 구현
  + 텍스트 스트리밍 -> 데이터객체 스트리밍 구현
[프로젝트] 프로덕션 안정성 확보(api, llm-proxy)
  + 에러 체계화로 빠른 원인 파악 및 해결 가능하도록 구현
  + 에러별 메세지를 스트리밍 처리 -> 화면 표출하도록 구현해 사용자 불만 최소화되도록 구현
```

### 1. 멀티 LLM 프로바이더 추상화 계층 설계(llm-proxy)
...
### 5. 24개 이상의 분석 에이전트 확장 가능한 설계

**문제**: AIReport, VOC, Dashboard, Heatmap, Docent, Schema, Geo 등 지속적으로 새로운 분석 유형이 추가되었습니다.

**해결**: 에이전트 기반 라우팅 아키텍처를 설계하여, 새 에이전트 추가 시 비즈니스 로직만 구현하면 되는 플러그인 구조를 만들었습니다.
...
**성과**: 단일 코드베이스에서 24개 이상의 분석 서비스를 관리하며, 새 에이전트 추가 시 개발 비용 최소화
...
### 7. 컨테이너 기반 마이크로서비스 인프라 구축

개발 편의를 위해 4개의 Docker 컨테이너를 오케스트레이션하는 인프라를 설계했습니다.
