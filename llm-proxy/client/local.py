...
@dataclass
class LocalClient:
    data: ChatPayload | ChatMCPPayload | EmbeddingsPayload
    ...
    async def get_messages(self):
        ...
        messages = [
            ...
            {"role": "assistant", "content": await self.get_system_prompt()}
        ]
    ...
    async def stream(self, is_retry: bool = False):
        try:
            ...
            logging.info(f"session_key: {self.data.session_key} | [client.llm.stream] REQEUST to LocalLLM")
            logging.info(f"session_key: {self.data.session_key} | [client.llm.stream] Address: {self.local_url}")
            args = {
                "model": "gpt-oss:20b",
                "messages": messages,
                "stream": True,
            ...
            if args["model"] not in LOCAL_REASONING_MODEL_LIST:
            ...
            async with asyncio.timeout(TIMEOUT) as timeout_cm:
                ...
                async for chunk in response:
                    ...
                    if is_first_res:
                        ...
                        response_dict["vendor"] = "LMSTUDIO"
                        is_first_res = False
                        timeout_cm.reschedule(None)
                        if self.data.time:
                            time_first_req = time()
                            elapsed = timedelta(seconds=int(time_first_req - self.data.time))
                            logging.info(f"session_key: {self.data.session_key} | [TIMETEST] ⏱️ TIME from LMSTUDIO: {elapsed}")
            ...
            logging.error(f"session_key: {self.data.session_key} | [client.local.LocalClient.stream] TIMEOUT ERROR: {TIMEOUT}")
            if not is_retry:
                raise FirstTryError()
            else:
                yield make_error_response()
        except Exception as e:
            logging.error(f"session_key: {self.data.session_key} | [client.local.LocalClient.stream] ERROR: {e}")
            if not is_retry:
                raise FirstTryError()
            else:
                yield make_error_response()
    ...
    async def stream_with_mcp(self, is_retry: bool = False):
        ...
        try:
                    ...
                    async for chunk in response:
                        ...
                        if is_first_res:
                            ...
                            if self.data.time:
                                time_first_req = time()
                                elapsed = timedelta(seconds=int(time_first_req - self.data.time))
                                logging.info(f"session_key: {self.data.session_key} | [TIMETEST] ⏱️ TIME from LMSTUDIO MCP: {elapsed}")
            ...
            logging.error(f"session_key: {self.data.session_key} | [client.local.LocalClient.stream_with_mcp] TIMEOUT ERROR: {TIMEOUT}")
            if not is_retry:
                raise FirstTryError()
            else:
                yield make_error_response()
        except Exception as e:
            logging.error(f"session_key: {self.data.session_key} | [client.local.LocalClient.stream_with_mcp] ERROR: {e}")
            if not is_retry:
                raise FirstTryError()
            else:
                yield make_error_response()
    ...
    async def stream_with_queue(self, is_retry: bool = False):
        try:
            ...
            logging.info(f"session_key: {self.data.session_key} | [client.local.LocalClient.stream_with_queue] service: {self.data.service} | is_retry: {is_retry} | REQUEST to LocalLLM")
            args = {
                "model": "gpt-oss:20b",
                "messages": messages,
                "stream": True,
            ...
            logging.error(f"session_key: {self.data.session_key} | [client.local.LocalClient.stream_with_queue] ERROR: {e}")
            if not is_retry:
                raise FirstTryError()
            else:
